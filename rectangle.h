...
public:
    ...
    // 当使用两个纹理贴图时
    // 当载入一个纹理贴图时, 所使用的着色器又有些不同
    Rectangle(int width, int height, const char *image1, const char *image2):
        positionx(0),
        positiony(0),
        shader("./rect1.vs", "./rect2.fs")
    {
        // 首先, 读入第一张纹理
        int imgw, imgh, nrChannels;
        stbi_set_flip_vertically_on_load(true);
        unsigned char *data = stbi_load(image1, &imgw, &imgh, &nrChannels, 0);
        // 生成纹理ID, 并绑定
        glGenTextures(1, &texture1);
        glBindTexture(GL_TEXTURE_2D, texture1);
        // 为纹理设置环绕过滤方式
        ...
        // 第二张纹理
        stbi_set_flip_vertically_on_load(true);
        data = stbi_load(image2, &imgw, &imgh, &nrChannels, 0);
        // 生成纹理ID, 并绑定
